...
namespace FawnAlgebra
{
    ...
    struct Vec
    {
        ...
        explicit constexpr Vec( const T& val ) noexcept : data {}
        {
            data.fill( val );
        }

        explicit constexpr Vec( std::initializer_list<T> args ) noexcept : data {}
        {
            // static_assert(args.size() == N, "argument list is incorrect size");
            std::ranges::copy( args, data.begin() );
        }

        constexpr Vec( const Vec& other ) : data {}
        {
            std::ranges::copy( other.data, data.begin() );
        }

        constexpr Vec( Vec&& other ) noexcept : data( std::move( other.data ) )
        {
        }
...
#define OVERLOAD_FUNC_VEC_VEC( func )                                                                                                                                                                                                                    \
    ...
    {                                                                                                                                                                                                                                                    \
        ...
        for ( std::size_t i {}; i < N; ++i )                                                                                                                                                                                                             \
        {                                                                                                                                                                                                                                                \
            result.data[ i ] = std::func( v[ i ], w[ i ] );                                                                                                                                                                                              \
        }                                                                                                                                                                                                                                                \
        return result;                                                                                                                                                                                                                                   \
    }
