...
export constexpr uint32 HashLittle( const void* key, std::size_t length, const uint32 initval ) noexcept
{
    ...
    if constexpr ( std::endian::native == std::endian::little )
    {
        ...
        {
            const auto* k = std::bit_cast<const uint32*>( key );/* read 32-bit chunks */
 ...
 */
        ...
        {
            const auto* k = std::bit_cast<const uint16*>( key );/* read 16-bit chunks */
            ...
            /*----------------------------- handle the last (probably partial) block */
            const auto* k8 = std::bit_cast<const uint8*>( k );
            switch ( length )
            {
    ...
    else
    {
        ...
        const auto* k = std::bit_cast<const uint8*>( key );
