...
namespace FawnAlgebra
{
    ...
    {
    public:
        constexpr explicit Random( uint32_t seed = time( nullptr ) & MAX_PERMUTATION_INDEX ) : m_offset { seed & MAX_PERMUTATION_INDEX }
        {
        }
        constexpr void InitState( uint32_t seed = time( nullptr ) & MAX_PERMUTATION_INDEX ) noexcept
        {
        ...
        constexpr uint32_t Next( uint32_t min, uint32_t max ) noexcept
        {
        ...
        constexpr uint32_t Next( uint32_t max ) noexcept
        {
            return Next( 0, max );
        }
        ...
        constexpr uint32_t Next64( uint64_t min, uint64_t max ) noexcept
        {
            const uint32_t diff   = max - min;
            const uint32_t value1 = permutation[ m_offset ];
            m_offset              = ++m_offset & MAX_PERMUTATION_INDEX;
        ...
        constexpr uint32_t Next64( uint64_t max ) noexcept
        {
            return Next64( 0, max );
        }
        ...
        constexpr float Perlin( float x, float y, float z = 0.0f ) noexcept
        {
            uint32_t X = static_cast<uint32_t>( std::floor( x ) ) & MAX_PERMUTATION_INDEX;// FIND UNIT CUBE THAT
            uint32_t Y = static_cast<uint32_t>( std::floor( y ) ) & MAX_PERMUTATION_INDEX;// CONTAINS POINT.
            uint32_t Z = static_cast<uint32_t>( std::floor( z ) ) & MAX_PERMUTATION_INDEX;
            ...
            uint32_t A  = permutation[ X ] + Y;// HASH COORDINATES OF
            uint32_t AA = permutation[ A ] + Z;// THE 8 CUBE CORNERS,
            uint32_t AB = permutation[ ( A + 1 ) & MAX_PERMUTATION_INDEX ] + Z;
            uint32_t B  = permutation[ ( X + 1 ) & MAX_PERMUTATION_INDEX ] + Y;
            uint32_t BA = permutation[ B ] + Z;
            uint32_t BB = permutation[ ( B + 1 ) & MAX_PERMUTATION_INDEX ] + Z;

            return lerp( w,
                         lerp( v,
                               lerp( u,
                                     grad( permutation[ AA & MAX_PERMUTATION_INDEX ], x, y, z ),      // AND ADD
                                     grad( permutation[ BA & MAX_PERMUTATION_INDEX ], x - 1, y, z ) ),// BLENDED
                               lerp( u,
                                     grad( permutation[ AB & MAX_PERMUTATION_INDEX ], x, y - 1, z ),        // RESULTS
                                     grad( permutation[ BB & MAX_PERMUTATION_INDEX ], x - 1, y - 1, z ) ) ),// FROM  8
                         lerp( v,
                               lerp( u,
                                     grad( permutation[ ( AA + 1 ) & MAX_PERMUTATION_INDEX ], x, y, z - 1 ),      // CORNERS
                                     grad( permutation[ ( BA + 1 ) & MAX_PERMUTATION_INDEX ], x - 1, y, z - 1 ) ),// OF CUBE
