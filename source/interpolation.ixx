...
{
inline constexpr auto toRadians { FawnAlgebra::pi<double> / 180.0 };
inline constexpr auto toDegree { 180.0 / FawnAlgebra::pi<double> };
...
constexpr T LerpAngleRad( const V& a, const V& b, const T& t ) noexcept
{
    ...
    // Calculate the shortest angle using the provided logic
    V shortestAngle {
        static_cast<V>( std::fmod( std::fmod( difference, FawnAlgebra::two_pi<V> ) + 3 * FawnAlgebra::pi<V>, FawnAlgebra::two_pi<V> )
                        - FawnAlgebra::pi<V> ) };
...
constexpr T EaseInOutExpo( const T& v ) noexcept
{
    return v == 0                   ? 0
        : v == 1                    ? 1
        : v < static_cast<T>( 0.5 ) ? static_cast<T>( std::pow( 2, 20 * v - 10 ) ) / 2
                                    : ( 2 - static_cast<T>( std::pow( 2, -20 * v + 10 ) ) ) / 2;
}
...
constexpr T EaseInOutBack( const T& v ) noexcept
{
    ...
    return v < 0.5 ? static_cast<T>( std::pow( 2 * v, 2 ) ) * ( ( c2 + 1 ) * 2 * v - c2 ) / 2
                   : ( static_cast<T>( std::pow( 2 * v - 2, 2 ) ) * ( ( c2 + 1 ) * ( v * 2 - 2 ) + c2 ) + 2 ) / 2;
}
...
constexpr T EaseInElastic( const T& v ) noexcept
{
    ...
    return v == 0 ? 0
        : v == 1  ? 1
                  : static_cast<T>( -std::pow( 2, 10 * v - 10 ) ) * FawnAlgebra::Sin( ( v * 10 - static_cast<T>( 10.75 ) ) * c1 );
}
...
constexpr T EaseOutElastic( const T& v ) noexcept
{
    ...
    return v == 0 ? 0
        : v == 1  ? 1
                  : static_cast<T>( std::pow( 2, -10 * v ) ) * FawnAlgebra::Sin( ( v * 10 - static_cast<T>( 0.75 ) ) * c1 ) + 1;
}
