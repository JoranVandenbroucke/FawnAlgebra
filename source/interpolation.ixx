...
{
inline constexpr auto toRadians{fawn_algebra::pi<double> / 180.0};
inline constexpr auto toDegree{180.0 / fawn_algebra::pi<double>};

export template <typename T>
constexpr T ToRadians(const T& degrees) noexcept
{
    return static_cast<T>(degrees * toRadians);
}

export template <typename T>
constexpr T ToDegree(const T& rad) noexcept
{
    return static_cast<T>(rad * toDegree);
}
...
export template <typename T>
constexpr T Grad(const std::uint32_t hash, const T& x, const T& y, const T& z) noexcept
{
    const std::uint32_t h{hash & 15U}; // CONVERT LO 4 BITS OF HASH CODE
...
constexpr T LerpAngleRad(const V& a, const V& b, const T& t) noexcept
{
    ...
    // Check if the angle difference is approximately 180 degrees
    if (std::abs(fawn_algebra::pi<V> - std::abs(difference)) <= epsilon)
    {
        return a + fawn_algebra::pi<V> * t;
    }

    // Calculate the shortest angle using the provided logic
    V shortestAngle{static_cast<V>(std::fmod(std::fmod(difference, fawn_algebra::two_pi<V>) + 3 * fawn_algebra::pi<V>, fawn_algebra::two_pi<V>) - fawn_algebra::pi<V>)};

    if (std::abs(shortestAngle) <= static_cast<T>(1e-6))
    {
        return a + fawn_algebra::two_pi<V> * t;
    }
...
constexpr T IlerpAngleRad(const V& a, const V& b, const V& v) noexcept
{
    ...
    if (std::abs(fawn_algebra::pi<V> - std::abs(difference)) <= epsilon)
    {
        return static_cast<T>((v - a) / fawn_algebra::pi<V>);
    }

    // Calculate the shortest angle using the provided logic
    const V shortestAngle{static_cast<V>(std::fmod(std::fmod(difference, fawn_algebra::two_pi<V>) + 3 * fawn_algebra::pi<V>, fawn_algebra::two_pi<V>) - fawn_algebra::pi<V>)};

    // if shortest angle is 0, return a
    if (std::abs(shortestAngle) <= static_cast<T>(1e-6))
    {
        return static_cast<T>((v - a) / fawn_algebra::two_pi<V>);
    }
    return static_cast<T>((v - a) / shortestAngle);
}
...
{
    return 1 - fawn_algebra::Cos(v * fawn_algebra::pi<T> / 2);
}
...
{
    return fawn_algebra::Sin(v * fawn_algebra::pi<T> / 2);
}
...
{
    return -(fawn_algebra::Cos(fawn_algebra::pi<T> * v) - 1) / 2;
}
...
{
    return v < static_cast<T>(0.5) ? (1 - static_cast<T>(std::sqrt(1 - static_cast<T>(std::pow(2 * v, 2))))) / 2
                                   : (static_cast<T>(std::sqrt(1 - static_cast<T>(std::pow(-2 * v + 2, 2)))) + 1) / 2;
}
...
{
    constexpr T c1{2 * fawn_algebra::pi<T> / 3};

    return v == 0 ? 0 : v == 1 ? 1 : static_cast<T>(-std::pow(2, 10 * v - 10)) * fawn_algebra::Sin((v * 10 - static_cast<T>(10.75)) * c1);
}
...
{
    constexpr T c1{2 * fawn_algebra::pi<T> / 3};
    return v == 0 ? 0 : v == 1 ? 1 : static_cast<T>(std::pow(2, -10 * v)) * fawn_algebra::Sin((v * 10 - static_cast<T>(0.75)) * c1) + 1;
}

export template <typename T>
constexpr T EaseInOutElastic(const T& v) noexcept
{
    constexpr T c1{2 * fawn_algebra::pi<T> / static_cast<T>(4.5)};

    return v == 0 ? 0
        : v == 1  ? 1
        : v < 0.5 ? -(static_cast<T>(std::pow(2, 20 * v - 10)) * fawn_algebra::Sin((20 * v - static_cast<T>(11.125)) * c1)) / 2
                  : static_cast<T>(std::pow(2, -20 * v + 10)) * fawn_algebra::Sin((20 * v - static_cast<T>(11.125)) * c1) / 2 + 1;
}
