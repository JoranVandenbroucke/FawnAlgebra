...
constexpr auto QuartilesQ1( const Container& a ) noexcept
{
    ...
    return static_cast<T>( std::size( a ) & 3 ) != 0
        ? a[ static_cast<std::size_t>( std::ceil( idx ) ) ]
        : ( a[ static_cast<std::size_t>( idx ) ] + a[ static_cast<std::size_t>( idx + 1 ) ] ) / 2;
}
...
constexpr auto quartilesQ2( const Container& a ) noexcept
{
    ...
    return static_cast<T>( std::size( a ) & 1 ) != 0
        ? a[ static_cast<std::size_t>( std::ceil( idx ) ) ]
        : ( a[ static_cast<std::size_t>( idx ) ] + a[ static_cast<std::size_t>( idx + 1 ) ] ) / 2;
}
