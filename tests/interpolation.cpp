...
{
    REQUIRE(LerpAngleRad(0.0f, fawn_algebra::two_pi<float>, 0.5f) == Catch::Approx(fawn_algebra::pi<float>));
}

TEST_CASE("Math: ilerpAngleRad", "[math]")
{
    REQUIRE(IlerpAngleRad(0.0f, fawn_algebra::two_pi<float>, fawn_algebra::pi<float>) == Catch::Approx(0.5f));
}

TEST_CASE("Math: remapAngleToValueRad", "[math]")
{
    REQUIRE(RemapAngleToValueRad(0.0f, fawn_algebra::two_pi<float>, 0.0f, 100.0f, fawn_algebra::pi<float>) == Catch::Approx(50.0f));
}

TEST_CASE("Math: remapAngleFromValueRad", "[math]")
{
    REQUIRE(RemapAngleFromValueRad(0.0f, 100.0f, 0.0f, fawn_algebra::two_pi<float>, 50.0f) == Catch::Approx(3.14159f));
}

TEST_CASE("Math: remapAngleRad", "[math]")
{
    REQUIRE(RemapAngleRad(0.0f, fawn_algebra::two_pi<float>, 0.0f, 100.0f, fawn_algebra::pi<float>) == Catch::Approx(-0.265483737f));
}
