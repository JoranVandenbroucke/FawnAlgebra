...
TEST_CASE("statistics")
{
    ...
    SECTION("normal distribution")
    {
        std::array sorted{array1};
        std::ranges::sort(sorted);
        REQUIRE_THAT(fawn_algebra::NormalProbabilityDensity(33, 30, 11, 3), Catch::Matchers::WithinAbs(0.13298076013381091, 1e-6));
        REQUIRE(fawn_algebra::NormalProbabilityDensity(sorted)
                == std::vector<std::pair<int, double>>{{-70, 6.2208328709615639E-211},
                                                       {0, 0.000000000000000000000069720292080873287},
                                                       {10, 0.0000000000050202026130564222},
                                                       {10, 0.0000000000050202026130564222},
                                                       {20, 0.00012126275693175315},
                                                       {20, 0.00012126275693175315},
                                                       {30, 0.13298076013381091},
                                                       {40, 0.017996988837729353},
                                                       {50, 3.0058032269314661e-7},
                                                       {60, 6.195422209527299e-16},
                                                       {80, 4.9469853104677929e-45},
                                                       {90, 1.9164609361902576e-65},
                                                       {100, 3.3706565508222219e-90}});
    }

    REQUIRE_THAT(fawn_algebra::ZScore(33, 30, 3), Catch::Matchers::WithinAbs(1.0, 1e-6));
    REQUIRE_THAT(fawn_algebra::PoissonDistribution(2, 0.61), Catch::Matchers::WithinAbs(0.1010904292, 1e-6));
    REQUIRE_THAT(fawn_algebra::ChiSquareTest(array1, array2), Catch::Matchers::WithinAbs(7, 1e-6));
}
